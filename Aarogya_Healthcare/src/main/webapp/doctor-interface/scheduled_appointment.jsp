			...
			for (int i = 1; i <= totalScheduledAppointments; i++) {		
				...
				<tr>
					...
					<td id="<%= rs.getString(1) %>">
						<button class="yes-button" onclick="visitedUpdate2(this.parentElement.id, 'yes')">Yes</button>
						<button class="no-button" onclick="visitedUpdate2(this.parentElement.id, 'no')">No</button>
					</td>
					<td style="text-align: center"><button class="delete-button" id="<%= rs.getInt(1) %>" onclick="loadApptFilterDashboard2(this.id)">Cancel</button></td>					
				</tr>
				<%
			}
			%>			
